...
class GSoutputs:
    def __init__(self, version, modelname, simtime, exptime, variablename, path, description, nosteps, noreal, weights, variables, timeseries):
...
def readFile(filename):
    ...
    for i in range(numVar):
        df = pd.read_csv(filename, skiprows=10+i*(file[7]+4), header=None, nrows=2)
        variables.append({'variable': df[0][0].split(":", 1)[1], 'unit': df[0][1].split(":", 1)[1]})
    ...
    df_compiled = ""
    for i in range(numVar):
        df = pd.read_table(filename, names=header, skiprows=14+i*(file[7]+4), nrows=file[7], sep='\t') 
    ...
    output = GSoutputs(file[0], file[1], file[2], file[3], file[4], file[5], file[6], file[7], file[8], file[9], file[10], file[11])
    return output
